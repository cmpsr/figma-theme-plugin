  ...
  public get() {
    this.traversePage((node: FrameNode) => {
      if (this.nodeStartsWithPrefix(node.name, THEME_PREFIXES.TEXTS_PAIRINGS.DEFAULT)) {
        const textPairingNodeNameNormalized = normalizeTextSuffixToken(node.name);
        ...
        const labelTextNode = (node.children[0] as InstanceNode).children[0] as TextNode;
        const labelTextNodeNameNormalized = normalizeTextSuffixToken(labelTextNode.name);

        const labelNodeToken = labelTextNodeNameNormalized.replace(
          new RegExp(`${THEME_PREFIXES.TEXTS.DESKTOP}|${THEME_PREFIXES.TEXTS.MOBILE}|${THEME_PREFIXES.TEXTS.DEFAULT}`),
          ''
        );
        const subLabelTextNode = (node.children[1] as InstanceNode).children[0] as TextNode;
        const subLabelTextNodeNameNormalized = normalizeTextSuffixToken(subLabelTextNode.name);

        const subLabelNodeToken = subLabelTextNodeNameNormalized.replace(
          new RegExp(`${THEME_PREFIXES.TEXTS.DESKTOP}|${THEME_PREFIXES.TEXTS.MOBILE}|${THEME_PREFIXES.TEXTS.DEFAULT}`),
          ''
        );
        const isDesktopText = node.name.startsWith(THEME_PREFIXES.TEXTS_PAIRINGS.DESKTOP);
        const isMobileText = node.name.startsWith(THEME_PREFIXES.TEXTS_PAIRINGS.MOBILE);
