...
export class Elevations extends Page {
  ...
  private parseRgbaNumber = (number: number) => Number((number * 255).toFixed());

  private parseEffectColor = (color: RGBA) =>
    `rgba(${this.parseRgbaNumber(color.r)}, ${this.parseRgbaNumber(color.g)}, ${this.parseRgbaNumber(color.b)}, ${
      Math.round(color.a * 100) / 100
    })`;

  private parseEffects = (effects: Array<Effect>) =>
    effects
      ?.map(({ offset, radius, spread, type, color }: DropShadowEffect | InnerShadowEffect) => {
  ...
  public get = (): Record<string, string> => {
    this.traversePage((node: SceneNode) => {
      if (this.nodeStartsWithPrefix(node.name, THEME_PREFIXES.ELEVATIONS)) {
        ...
        const elevationStyle = figma.getStyleById(elevationNode.effectStyleId) as BaseStyle & EffectStyle;
        const boxShadow = this.parseEffects(elevationStyle.effects as Effect[]);
        const elevationToken = elevationNode.name.replace(THEME_PREFIXES.ELEVATIONS, '');
